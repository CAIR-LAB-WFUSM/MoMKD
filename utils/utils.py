...
def plot_loss_curves(history, plot_save_path):
    # This is a sample implementation, adjust keys ('train_loss', etc.) as needed
    colors = {'loss': ['#1f77b4', '#ff7f0e']} # Example colors
    
    epochs = np.arange(1, len(history['train_loss']) + 1)
    
    plt.figure(figsize=(12,8))

    for loss_name in ['loss']: # Assuming you are tracking a single 'loss'
        train_key = f'train_{loss_name}'
        val_key = f'val_{loss_name}' # Assuming you also have validation loss

        if train_key in history:
            # --- THIS IS THE FIX ---
            train_values = history[train_key]
            # Check if the first item is a tensor; if so, convert the whole list
            if train_values and isinstance(train_values[0], torch.Tensor):
                train_values = [t.cpu().item() for t in train_values]
            # --- END OF FIX ---
            plt.plot(epochs, train_values, label=f'Train {loss_name.capitalize()}', color=colors[loss_name][0], marker='o', linestyle='-', markersize=4)

        if val_key in history:
            # --- APPLY THE SAME FIX FOR VALIDATION LOSS ---
            val_values = history[val_key]
            if val_values and isinstance(val_values[0], torch.Tensor):
                val_values = [t.cpu().item() for t in val_values]
            # --- END OF FIX ---
            plt.plot(epochs, val_values, label=f'Val {loss_name.capitalize()}', color=colors[loss_name][1], marker='x', linestyle='--', markersize=4)
...
def collate_MIL_odx(batch):
    """
    Correctly collates batches for the ODx task.
    Batch size is typically 1, as each item can have a variable number of patches.
    """
    # Each 'item' in the batch is a tuple (wsi_features, rna_features, label)
    
    # We don't stack wsi_features because each bag has a different number of patches.
    # The training loop will process one bag at a time.
    wsi_features = [item[0] for item in batch]
    
    # RNA features and labels CAN be stacked into a batch.
    rna_features = torch.stack([item[1] for item in batch], dim=0)
    labels = torch.LongTensor([item[2] for item in batch])
    slide_ids = [item[3] for item in batch]
    # Since batch_size=1, wsi_features will be a list containing one tensor.
    # We'll handle this in the training loop.
    return wsi_features, rna_features, labels, slide_ids
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False, mode='coattn', batch_size=1):
    ...
    """
    if hasattr(split_dataset, 'omic_dir'): # A unique attribute of our ODx dataset
        collate = collate_MIL_odx
    elif mode == 'coattn':
